...
export default function AuthenticatedLayout({
    ...
    const isActive = (item: NavItem) => {
        ...
        if (item.routeName.endsWith('.*')) {
            return current.startsWith(item.routeName.slice(0, -2));
        }
        return current === item.routeName;
    };

    const unreadCount = usePage().props.unreadNotificationsCount as number;
    ...
    return (
        <div className="min-h-screen bg-background">
            ...
            {/* Sidebar */}
            <aside
                className={`fixed inset-y-0 left-0 z-50 flex w-64 flex-col border-r border-border bg-sidebar text-sidebar-foreground shadow-lg shadow-black/5 transition-transform duration-300 ease-in-out lg:translate-x-0 ${
                    sidebarOpen ? 'translate-x-0' : '-translate-x-full'
                }`}
            >
                {/* Logo */}
                <div className="flex h-16 shrink-0 items-center gap-3 border-b border-sidebar-accent/10 px-5">
                    <Link href="/" className="flex items-center gap-2.5">
                        <div className="flex h-8 w-8 items-center justify-center rounded-lg bg-sidebar-accent/15">
                            <ApplicationLogo className="h-5 w-5 fill-current text-sidebar-accent" />
                        </div>
                        <span className="text-sm font-bold text-sidebar-foreground">E-Learning MIS</span>
                    </Link>
                </div>
                    ...
                    {sections.map((section) => {
                        ...
                        return (
                            <div key={section.label} className="mb-5">
                                <div className="mb-1.5 px-3 text-[10px] font-semibold uppercase tracking-[0.12em] text-sidebar-foreground/40">
                                    {section.label}
                                </div>
                                                ...
                                                <Link
                                                    ...
                                                    className={`group relative flex items-center gap-3 rounded-lg px-3 py-2.5 text-sm font-medium transition-all duration-200 ${
                                                        active
                                                            ...
                                                            : 'text-sidebar-foreground/60 hover:bg-sidebar-accent/8 hover:text-sidebar-foreground/90'
                                                    }`}
                                                >
                                                    {/* Active indicator */}
                                                    {active && (
                                                    ...
                                                    <span className={`flex h-5 w-5 items-center justify-center transition-transform duration-200 group-hover:scale-110 ${
                                                        active ? 'text-sidebar-accent' : 'text-sidebar-foreground/40'
                                                    }`}>
                ...
                {/* User footer */}
                <div className="shrink-0 border-t border-sidebar-accent/10 px-3 py-4">
                    <div className="flex items-center gap-3 rounded-lg px-3 py-2.5 transition-colors hover:bg-sidebar-accent/5">
                        <div className="flex h-8 w-8 shrink-0 items-center justify-center rounded-full bg-sidebar-accent/15 text-xs font-bold text-sidebar-accent">
                            {user?.name?.charAt(0).toUpperCase()}
                        </div>
                        <div className="flex-1 truncate">
                            <p className="text-sm font-medium text-sidebar-foreground truncate">{user?.name}</p>
                            <p className="text-xs text-sidebar-foreground/40 capitalize">{roleBadge}</p>
                        </div>
                        <Link
                            href={route('logout')}
                            method="post"
                            as="button"
                            className="flex h-7 w-7 shrink-0 items-center justify-center rounded-md text-sidebar-foreground/30 transition-colors hover:bg-sidebar-accent/10 hover:text-sidebar-foreground/70"
                            title="Log out"
                        >
            ...
            <div className="lg:pl-64">
                ...
                <header className="sticky top-0 z-30 flex h-16 items-center gap-4 border-b border-border bg-background/95 backdrop-blur supports-[backdrop-filter]:bg-background/60 px-4 sm:px-6">
                    ...
                    <div className="hidden sm:relative sm:block" ref={userDropdownRef}>
                        <button
                            type="button"
                            onClick={() => setShowUserDropdown(!showUserDropdown)}
                            className="flex items-center gap-2.5 rounded-lg border border-border px-3 py-2 text-sm font-medium text-foreground transition-colors hover:bg-muted"
                        ...
                        {showUserDropdown && (
                            <div className="absolute right-0 z-50 mt-2 w-56 origin-top-right rounded-lg border border-border bg-popover shadow-lg ring-1 ring-black/5 focus:outline-none">
                                <div className="border-b border-border px-4 py-3">
                                    <p className="text-sm font-medium text-popover-foreground">{user?.name}</p>
                                    <p className="truncate text-xs text-muted-foreground">{user?.email}</p>
                                    <span className="mt-1.5 inline-flex rounded-full bg-primary/10 px-2 py-0.5 text-xs font-medium text-primary capitalize">
                                        {roleBadge}
                                    </span>
                                </div>
                                <div className="py-1">
                                    <Link
                                        ...
                                        className="flex items-center gap-2 px-4 py-2 text-sm text-popover-foreground transition-colors hover:bg-accent hover:text-accent-foreground"
                                    >
                                    ...
                                    <Link
                                        ...
                                        as="button"
                                        onClick={() => setShowUserDropdown(false)}
                                        className="flex w-full items-center gap-2 px-4 py-2 text-left text-sm text-popover-foreground transition-colors hover:bg-accent hover:text-accent-foreground"
                                    >
